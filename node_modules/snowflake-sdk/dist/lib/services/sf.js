...
const AuthOauthAuthorizationCode = require('../authentication/auth_oauth_authorization_code');
const sfParams = require('../constants/sf_params');
const { getMinicoreStatus } = require('../minicore');
const { getOsDetails } = require('../telemetry/os_details');
const { getApplicationPath } = require('../telemetry/application_path');
const { buildInbandTelemetryRequest } = require('../telemetry/inband_telemetry');
const { getDetectedPlatforms } = require('../telemetry/platform_detection');
const { getLibcDetails } = require('../telemetry/libc_details');
const { getSpcsToken } = require('../authentication/spcs_token');
// NOTE:
// Ideally login-request should utilize useSnowflakeRetryMiddleware, but the migration effort
// isn't justified given the upcoming migration to Universal Driver.
function isRetryableError(err) {
    if (err.response) {
        return Util.isRetryableHttpError(err.response, false);
    }
    else {
...
function StateAbstract(options) {
    ...
    function sendHttpRequest(requestOptions, httpClient, auth) {
        ...
        const realRequestOptions = {
            ...
            params: params,
            useSnowflakeRetryMiddleware: requestOptions.useSnowflakeRetryMiddleware,
            callback: async function (err, response, body) {
                ...
                if (err) {
                    ...
                    if (body && !body.success) {
                        ...
                        if (needsReauth) {
                            Logger.getInstance().debug('Authentication rejected by GS (code=%s). Reauthenticating.', body.code);
 ...
 * @inheritDoc
 */
StateConnecting.prototype.continue = async function () {
    ...
    const libcDetails = getLibcDetails();
    ...
    const clientInfo = {
        ...
        CLIENT_ENVIRONMENT: {
            ...
            ISA: process.arch,
            CORE_VERSION: minicoreStatus.version,
            CORE_FILE_NAME: minicoreStatus.binaryName,
            CORE_LOAD_ERROR: minicoreStatus.errorType,
            OS_DETAILS: getOsDetails(),
            PLATFORM: platforms,
            LIBC_FAMILY: libcDetails.family,
            LIBC_VERSION: libcDetails.version,
            ...this.connectionConfig.getClientEnvironment(),
            ...(clientApplication ? { APPLICATION: clientApplication } : {}),
    ...
    if (spcsToken) {
        json.data.SPCS_TOKEN = spcsToken;
    }
    ...
    const requestCallback = async function (err, body) {
        ...
        if (!err) {
            ...
            if (minicoreStatus.errorDetails) {
                const telemetryRequest = buildInbandTelemetryRequest(connectionConfig, 'minicore_error', {
                    binaryName: minicoreStatus.binaryName,
                    message: minicoreStatus.errorDetails.message,
                    stack: minicoreStatus.errorDetails.stack,
                });
                parent
                    .createSessionTokenRequest(telemetryRequest)
                    .sendAsync()
                    .catch((error) => {
                    Logger.getInstance().debug(`Failed to send telemetry event: %s`, error);
                });
            }
            // TODO(SNOW-3548350): remove with the in-band telemetry emission (target: 2026-11-30).
            if (!process.env.SF_TELEMETRY_DISABLE_CONNECTION_SHAPE) {
                const shapeRequest = buildInbandTelemetryRequest(connectionConfig, 'client_connection_identifier_shape', connectionConfig.connectionShapeForTelemetry);
                parent
                    .createSessionTokenRequest(shapeRequest)
                    .sendAsync()
                    .catch((error) => {
                    Logger.getInstance().debug(`Failed to send telemetry event: %s`, error);
                });
