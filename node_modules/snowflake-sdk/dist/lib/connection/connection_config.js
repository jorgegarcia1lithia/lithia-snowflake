...
const { buildConnectionShapePayload } = require('../telemetry/connection_identifier_shape');
...
function consolidateHostAndAccount(options) {
    ...
    if (Util.exists(options.accessUrl)) {
        ...
        try {
            const parsedUrl = new URL(options.accessUrl);
            ...
            Errors.checkArgumentValid(false, ErrorCodes.ERR_CONN_CREATE_INVALID_ACCESS_URL);
        }
    }
...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
    ...
    this.connectionShapeForTelemetry = buildConnectionShapePayload(options);
    ...
    let proxy;
    if (Util.exists(proxyHost) || Util.exists(proxyPort)) {
        proxy = {
            host: proxyHost,
            port: proxyPort,
            user: proxyUser,
    ...
    this.browserRedirectPort = options.browserRedirectPort ?? 0;
